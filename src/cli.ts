...
function printSuccessScreen(copied: number, skipped: number, version: string, targetDir: string): void {
  ...
  console.log('\n  RESOURCES\n');
  console.log('  NPM       https://npmjs.com/package/deepagents-cli-claude-skill');
  console.log('  GitHub    https://github.com/Gitmaxd/deepagents-cli-claude-skill\n');
  console.log(DIVIDER);
  console.log('');
}
...
const init = defineCommand({
  ...
  args: {
    ...
    personal: {
      type: 'boolean',
      description: 'Install as a personal skill (~/.claude/skills/) instead of workspace',
      default: false,
    },
  },
  async run({ args }) {
    ...
    const skillDir = args.personal
      ...
      : join(baseDir, '.claude', 'skills', SKILL_NAME);
    ...
    if (await pathExists(skillDir) && !args.force) {
      console.log(`  .claude/skills/${SKILL_NAME} already exists`);
      ...
      await copy(templatesDir, skillDir, {
        ...
        filter: async (src, dest) => {
          ...
          if (await pathExists(dest)) {
            const relativePath = dest.replace(skillDir, `.claude/skills/${SKILL_NAME}`);
